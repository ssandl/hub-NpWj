"""
CrossEncoder 训练脚本 — BQ Corpus 版本（交互型文本匹配）

与 src/train_crossencoder.py 的差异：
  - 默认 DATA_DIR 指向 data/bq_corpus
  - checkpoint 与日志统一加 bq_ 前缀，避免覆盖 AFQMC 训练产物
  - 训练逻辑（CrossEntropyLoss + argmax 评估）完全一致

教学重点：
  1. CrossEncoder vs BiEncoder：让两句话在 BERT 每一层都交互
  2. 输入格式：[CLS] s1 [SEP] s2 [SEP]，token_type_ids 区分两段
  3. CrossEncoder 评估与普通分类完全相同（argmax 即为预测），无需阈值搜索

使用方式：
  python train_crossencoder_bq.py                                # 默认 4 层，3 epoch
  python train_crossencoder_bq.py --num_hidden_layers 12 --epochs 5   # 全量训练
  python train_crossencoder_bq.py --epochs 1 --batch_size 32          # 冒烟测试
...
def train_one_epoch(model, loader, optimizer, scheduler, criterion,
    ...
    """CrossEncoder 训练 loop（与 AFQMC 版完全相同）"""
    model.train()
...
def main():
    ...
    print(f"设备: {device}")
    print(f"数据集: BQ Corpus  |  路径: {args.data_dir}")
    ...
    # BQ Corpus 类别分布与 AFQMC 类似，正样本略少于负样本；CrossEntropyLoss 不加权
    criterion = nn.CrossEntropyLoss()

    # ── 训练循环 ──────────────────────────────────────────────────────────
    # BQ Corpus 专属 checkpoint / 日志路径（与 AFQMC 物理隔离）
    ...
    print(f"\n运行评估：python evaluate_bq.py --model_type crossencoder --ckpt {ckpt_path}")


# ── 参数解析 ──────────────────────────────────────────────────────────────

def parse_args():
    parser = argparse.ArgumentParser(description="CrossEncoder 训练 — BQ Corpus")
