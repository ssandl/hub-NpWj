...
输出：
  outputs/figures1/entity_distribution.png   各类实体频次分布
  outputs/figures1/text_length_distribution.png  文本长度分布
...
def load_split(split: str) -> list:
    ...
    with open(path, "r", encoding="utf-8") as f:
        return json.load(f)
"""
概述：该函数用于收集文本记录的统计信息，包括实体类型计数、实体长度、文本长度、每句实体数量以及按类型分类的实体列表。
参数：
records: 一个包含多个文本记录的列表，每个记录是一个字典，至少包含"text"键，可选包含"label"键
返回值：
一个字典，包含以下键：
entity_type_counts: 实体类型计数
entity_lengths: 实体长度列表
text_lengths: 文本长度列表
entity_per_sentence: 每句实体数量列表
entities_by_type: 按类型分类的实体列表
"""
def collect_stats(records: list) -> dict:
    ...
    for row in records:
        ...
        while i < len(tags):
            tag = tags[i]
            if tag.startswith("B-"):
                etype = tag[2:]  # 提取实体类型，如 B-PER -> PER
                start_idx = i
                # 查找连续的 I- 标签以确定实体边界
                while i + 1 < len(tags) and tags[i + 1] == f"I-{etype}":
                    i += 1
                end_idx = i
