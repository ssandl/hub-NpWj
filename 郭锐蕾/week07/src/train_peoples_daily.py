"""
BERT NER 训练脚本 - 适配 Peoples Daily 数据集

教学重点：
  1. 直接处理 token-level BIO 标注格式（不同于 cluener 的 span 格式）
  2. 三种实体类型：PER（人名）、ORG（组织机构）、LOC（地点）
  3. --use_crf 参数：支持 BERT+Linear 和 BERT+CRF 两种模型
  4. 分层学习率和 Linear Warmup
...
def parse_args():
    parser = argparse.ArgumentParser(description="训练 BERT NER 模型（Peoples Daily）")
