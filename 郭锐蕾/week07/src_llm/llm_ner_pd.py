...
适配 Peoples Daily 数据集

教学重点：
  1. Peoples Daily 数据格式转换：token-level BIO → JSON 实体列表
  2. Loss masking：只在 JSON 输出部分计算 loss
  3. LoRA 高效微调 vs 全量微调

使用方式：
  python train_sft_pd.py                        # LoRA 微调（默认）
  python train_sft_pd.py --num_train 2000       # 快速演示
  python train_sft_pd.py --epochs 1             # 快速验证
...
def record_to_target(record: dict) -> str:
    """把 Peoples Daily 格式转为 SFT 目标 JSON 字符串。
    
    输入：{"tokens": [...], "ner_tags": [...]}
    输出：'{"entities": [{"text": "...", "type": "..."}]}'
    """
...
class SFTDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # loss mask：prompt 全 -100，只在 JSON 部分计算 loss
...
def parse_args():
    parser = argparse.ArgumentParser(description="LLM SFT NER 训练（Peoples Daily）")
...
def main():
    ...
    ckpt_dir   = output_dir / ("sft_full_ckpt_pd" if args.full_ft else "sft_adapter_pd")
    ...
    # 保存训练日志
    log_tag  = "full_ft_pd" if args.full_ft else "sft_pd"
    ...
    print(f"\n下一步：python evaluate_sft_pd.py 查看 entity F1")
