...
使用方式：
  python train_sft_lcqmc.py                        # LoRA，5000 条快速演示（默认）
  python train_sft_lcqmc.py --num_train -1         # LoRA，全部数据
  python train_sft_lcqmc.py --epochs 1             # 快速验证流程
...
def parse_args():
    parser = argparse.ArgumentParser(description="LLM SFT 文本匹配训练（LCQMC 数据集）")
...
def main():
    ...
    ckpt_dir   = output_dir / ("sft_lcqmc_full_ckpt" if args.full_ft else "sft_lcqmc_adapter")
    ckpt_dir.mkdir(parents=True, exist_ok=True)

    device   = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    mode_str = "全量微调" if args.full_ft else "LoRA 微调"
    print(f"使用设备: {device}  |  微调模式: {mode_str}  |  数据集: LCQMC")
    ...
    # ── 保存训练日志 ──────────────────────────────────────────────────────────
    log_tag  = "sft_lcqmc_full" if args.full_ft else "sft_lcqmc"
