...
使用方式：
  ...
  python llm_compare.py --data_dir ../data/lcqmc --num_samples 50 --model qwen-plus
...
def main():
    ...
    print(f"数据集: {dataset_name} ({data_path.name})  样本数: {len(samples)}")
    ...
    print(f"\n{'='*55}")
    print(f"LLM 评估结果（{args.model}，{dataset_name} 数据集，{len(samples)} 条样本）")
    ...
    log_dir = ROOT / "week08作业" / "outputs" / "logs"
    log_dir.mkdir(parents=True, exist_ok=True)
    out_path = log_dir / f"llm_compare_{dataset_name}_results.json"
    save_data = {
        "model": args.model,
