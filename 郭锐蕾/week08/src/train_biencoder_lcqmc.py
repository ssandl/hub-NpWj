"""
BiEncoder 训练脚本（表示型文本匹配，Sentence-BERT 架构）- LCQMC 数据集
...
使用方式：
  ...
  # 自定义参数
  python train_biencoder_lcqmc.py --loss cosine --pool mean --num_hidden_layers 4 --epochs 3
...
def main():
    ...
    # ── 训练循环 ──────────────────────────────────────────────────────────
    ckpt_name = f"biencoder_lcqmc_{args.loss}_best.pt"
    ...
    # ── 训练完成，保存日志 ────────────────────────────────────────────────
    log_path = LOG_DIR / f"biencoder_lcqmc_{args.loss}_log.json"
...
def parse_args():
    parser = argparse.ArgumentParser(description="BiEncoder 训练（表示型文本匹配）- LCQMC 数据集")
